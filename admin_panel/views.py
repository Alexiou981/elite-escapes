...
def admin_dashboard(request):
    ...
    return render(
        request, "admin_panel/dashboard.html", {"packages": packages}
    )
...
def add_package(request):
    if request.method == "POST":
            ...
            if images:
                for image_file in images:
                    PackageImages.objects.create(
                        package=package, image=image_file
                    )
        ...
        messages.error(
            request,
            "There was an error adding the package. "
...
def edit_package(request, package_id):
    ...
    if request.method == "POST":
        ...
        if "remove_cover" in request.POST and package.image:
        ...
        if delete_image_ids:
            for image_id in delete_image_ids:
                ...
                image_path = os.path.join(
                    settings.MEDIA_ROOT, str(image_obj.image)
                )
                if os.path.exists(image_path):
                    os.remove(image_path)
                image_obj.delete()
        ...
        if form.is_valid():
            ...
            if new_images:
                ...
                messages.success(
                    request, "New additional images uploaded successfully."
                )
    ...
    return render(
        request, "admin_panel/edit_package.html", {"form": form, "package": package}
    )
